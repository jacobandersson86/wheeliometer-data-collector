...
def fuse_pose(
    ...
    accel_deadband_xy_m_s2 = 0.18
    accel_deadband_z_m_s2 = 0.30
    gravity_axis_linear_gain = 0.10

    velocity_leak_tau_xy_s = 3.0
    velocity_leak_tau_z_s = 1.2
    ...
    for sample, orientation in zip(sample_list, orientation_samples):
        ...
        if gravity_norm > 1e-6:
            ...
            suppress = (1.0 - gravity_axis_linear_gain) * linear_along_gravity
            linear_world_m_s2[0] -= suppress * gx_hat
            linear_world_m_s2[1] -= suppress * gy_hat
            linear_world_m_s2[2] -= suppress * gz_hat

        linear_world_m_s2[0] = _apply_component_deadband(linear_world_m_s2[0], accel_deadband_xy_m_s2)
        linear_world_m_s2[1] = _apply_component_deadband(linear_world_m_s2[1], accel_deadband_xy_m_s2)
        linear_world_m_s2[2] = _apply_component_deadband(linear_world_m_s2[2], accel_deadband_z_m_s2)
        ...
        if is_still or not translation_enabled:
            ...
            leak_xy = dt / (velocity_leak_tau_xy_s + dt)
            leak_z = dt / (velocity_leak_tau_z_s + dt)
            velocity_m_s[0] *= 1.0 - leak_xy
            velocity_m_s[1] *= 1.0 - leak_xy
            velocity_m_s[2] *= 1.0 - leak_z

            if abs(velocity_m_s[2]) < 0.03:
                velocity_m_s[2] = 0.0
